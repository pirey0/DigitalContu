...
public class VisualBoard3D : MonoBehaviour
{
    ...
    {
        visualTiles = new VisualTile3D[game.Board.Width, game.Board.Height];

        for (int y = 0; y < visualTiles.GetLength(1); y++)
        {
            for (int x = 0; x < visualTiles.GetLength(0); x++)
            {
                visualTiles[x, y] = Instantiate(tilePrefab, new Vector3(x, 0, y), Quaternion.identity).GetComponent<VisualTile3D>();
